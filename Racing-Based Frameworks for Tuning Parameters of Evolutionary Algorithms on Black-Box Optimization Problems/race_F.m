...
ratio_cost = zeros(1, length(func_names));
for func_num = 1:13
    ...
    for d = 3
        ...
        savepath =  ['result_race', filesep, 'F', filesep, 'dim_', num2str(dim), filesep, func_names{func_num}];
        if ~isdir(savepath)
            mkdir(savepath);
        end
        ...
        %         savebestFitAll('F_result', savepath, 40, treatment, func_names, func_num, mutationTypes, crossoverTypes, F, CR, NP, options);
        
        bestFitEachFuncStr = load([savepath, filesep, 'bestFitEachFunc.mat']);
        ...
        while block_count <= runEnd && sum(treatment) > 1
            ...
            if block_count >= 2
                ...
                [p_f, Q, Rmean, control_id, p_corrected, reject, best_ids] = ...
                    f_conover_control(candidates, 0, alpha);
        ...
        ratio_cost(func_num) = 100 * sum(op_left_num) / (block_sum * length(treatment));  % cost of F-Race / cost of benchmark
        figure('Visible', 'off');
        plot(op_left_num, 'b--x','markersize', 4);
        xlabel('Restarts');
        ylabel('Left Parameter Configurations');
        title( 'Procedure of F-Race');
        grid on;
        print([savepath, filesep, 'F-Race'], '-depsc', '-r1200');
        close;
        
    end
end
csvwrite( ['result_race', filesep, 'F', filesep, 'ratio_cost.csv'], ratio_cost);