...
for func_num = 1:13
    ...
    for d = 3
        ...
        savepath =  ['result_race', filesep, 'KW', filesep, 'dim_', num2str(dim), filesep, func_names{func_num}];
        if ~isdir(savepath)
            mkdir(savepath);
        end
        ...
        title( 'Procedure of KW-Race');
        grid on;
        print([savepath, filesep, 'KW-Race'], '-depsc', '-r1200');
        close;
        
    end
end
csvwrite( ['result_race', filesep, 'KW', filesep, 'ratio_cost.csv'], ratio_cost);