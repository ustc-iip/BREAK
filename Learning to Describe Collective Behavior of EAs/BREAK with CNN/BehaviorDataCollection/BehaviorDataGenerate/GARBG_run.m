function gval = GARBG_run(save_func_name, func_num, algo_name, maxFEs, run, NP, D, inilb, iniub, lb, ub, initial_pop, savePop_step, shift_flag)
...
%     xoverKids(ind) = inilb + (iniub - inilb) .* rand(1, length(ind));
    ...
    gbest = x(index, :);   % best individual of each generation
%  fprintf(1, 'generation = %d, FEs = %d, sigma = %f, bestSolution = %e\n', generation, FEs, sigma, g_val);
	% flags
    if mod(FEs, savePop_step) == 0
        savePopFit = 1;
    else
