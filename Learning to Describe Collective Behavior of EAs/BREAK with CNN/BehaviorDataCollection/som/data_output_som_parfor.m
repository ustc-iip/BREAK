...
% w = 1;
for i =[6:10,14]                  %  [1,2,5,6,7,9]         %1:length(functionname)
    functionname_each = functionname{i};
    for j =1:length(algorithmname)       %1:length(algorithmname)
        algorithmname_each = algorithmname{j};
       parfor k=1: 10000                     %length(run_timenames)
            run_times_each=['run_' num2str(k)]                 %run_timenames{k};
            run_data_output_som(Map, inputhome, outputhome, functionname_each, algorithmname_each,run_times_each,num_unit);
        end
    end
    delete(gcp);
end
